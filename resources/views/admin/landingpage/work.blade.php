                ...
                @auth
                    ...
                    <li class="nav-item mr-1">
                        <button data-toggle="modal" data-target="#modal-create-landingpage"  class="btn  btn-second round btn-publish" onclick="updateLandinpageTemp('admin');">
                            @if(!empty($mode)) Sửa template @else Tạo template @endif
                        </button>
                        @push('outer')
                        <x-modal id="modal-create-landingpage">
                            ...
                            @if(!empty($mode))
                                <div class="modal-content">
                                    <form action="{{route('templates.update',@$landingpage->id)}}" method="post">
                                        @csrf
                                        {{ method_field('PUT') }}
                                        ...
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="exampleModalLabel">Cập nhật template</h5>
                                        </div>
                            ...
                            @else
                                <div class="modal-content">
                                    <form action="{{route('templates.store')}}" method="post">
                                        ...
                                        <input type="hidden" name="device" value="{{@Request::route('dv') ?? 'm'}}">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="exampleModalLabel">Tạo template mẫu</h5>
                                        </div>
                                        <div class="modal-body">
                                            <div class="form-group ">
                                                <label for="">Tên :</label>
                                            ...
                                            <div class="form-group ">
                                                <x-select.categories name="category_id" required="required" class="form-control font-medium-2" prefix="landingpage" selected="" />
                                            </div>
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
